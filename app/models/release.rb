...
class Release < ActiveRecord::Base
  ...
  def changeset
    @changeset ||= Changeset.new(project.user_repo_part, previous_release.try(:commit), commit)
  end
  ...
  def contains_commit?(other_commit)
    ...
    ['behind', 'identical'].include?(GITHUB.compare(project.user_repo_part, commit, other_commit).status)
  rescue Octokit::NotFound
    false
  rescue Octokit::Error => e
    Airbrake.notify(e, parameters: { github_repo: project.user_repo_part, commit: commit, other_commit: other_commit})
